...
```
floor = 時刻ありで今日の期限がまだ来ていなければ「昨日」、そうでなければ「今日」
base  = max(floor, 現在の回のdue_date)      // 期限切れ完了でも過去分を生成しないための下駄
daily:          base + 1日
...
**`floor` の意図（2026-09-16追加）**: 判定を日付だけで行うと、23:00期限のタスクを日付をまたいで完了した時に「今日はもう消化済み」と扱われ、**まだ来ていない今夜の回が丸ごと飛んでいた**。時刻がある回は「期限そのもの」で過ぎたかを判断する。時刻なしは従来どおり今日より後（完了した直後に同じタスクが今日へ生え直すのを避けるため）。

具体例:
- 23:00のタスクを翌0:10に完了 → 次回は「今夜23:00」（今日の分は飛ばさない）
- 23:00のタスクを今夜23:30に完了 → 次回は「明日」（今夜はもう過ぎた）
...
| ケース | 挙動 |
|---|---|
| 期限切れのまま完了 | 4.3の`base`により、期限がまだ過ぎていない直近1回だけ生成。過去分は永久に生成されない |
